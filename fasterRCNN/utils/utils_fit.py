...
from YOLOv5.utils import get_lr
...
def fit_one_epoch(model, train_util, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir):
    ...
    if len(loss_history.val_loss) <= 1 or (val_loss / epoch_step_val) <= min(loss_history.val_loss):
        print('Save best model to best_epoch_weights_fasterrcnn_resnet50.pth')
        torch.save(model.state_dict(), os.path.join(save_dir, "best_epoch_weights_fasterrcnn_resnet50.pth"))
